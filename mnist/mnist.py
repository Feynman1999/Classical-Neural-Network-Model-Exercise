...
train_db = datasets.MNIST('../data', train=True, download=True,
                   transform=transforms.Compose([
                        transforms.ToTensor(),
                        transforms.Normalize((0.1307,), (0.3081,))  # feature scaling 均值 方差
                   ]))
...
train_db, val_db = torch.utils.data.random_split(train_db, [50000, 10000])  # 手动划分val
print('train_db:', len(train_db), 'val_db:', len(val_db))
train_loader = torch.utils.data.DataLoader(train_db, batch_size=batch_size, shuffle=True)
val_loader = torch.utils.data.DataLoader(val_db, batch_size=batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(test_db, batch_size=batch_size, shuffle=True)

print(len(val_loader), len(train_loader.dataset))
...
def train():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            loss = criteon(logits, target) + 0.00001*regularization_loss
            ...
            if(batch_idx % 50 == 0):  # 每50*batchsize 个sample打印一次
                ...
                # viz.images(data.view(-1, 1, 28, 28), win='image')
